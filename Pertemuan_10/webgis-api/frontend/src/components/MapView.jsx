...
// Gabung bounds fasilitas + kotak deteksi
function FitBoundsCombined({ geojsonData, detectionGeoJSON, showDetection }) {
  const map = useMap()
  ...
  useEffect(() => {
    ...
    if (geojsonData?.features?.length) {
      fg.addLayer(L.geoJSON(geojsonData))
    }
    const detShown =
      showDetection &&
      detectionGeoJSON?.features?.some((f) => f.geometry)
    if (detShown) {
      fg.addLayer(L.geoJSON(detectionGeoJSON))
    }
  ...
  }, [geojsonData, detectionGeoJSON, showDetection, map])

  return null
}

function popupDetectionHtml(props) {
  const nm = props.class_name ?? props.class ?? '?'
  const cf = props.confidence != null ? (props.confidence * 100).toFixed(1) : '?'
  return `
    <div class="popup-card">
      <div class="popup-header">
        <div class="popup-icon" style="background: rgba(245,158,11,0.2); color: #f59e0b;">🎯</div>
        <div>
          <div class="popup-nama">${nm}</div>
          <span class="popup-jenis" style="background: rgba(245,158,11,0.2); color: #f59e0b;">YOLO • ${cf}% conf</span>
        </div>
      </div>
    </div>
  `
...
function MapView({ geojsonData, loading, detectionGeoJSON, showDetectionLayer }) {
  const geoJsonRef = useRef(null)

  const defaultCenter = [-5.38, 105.27] // Lampung Selatan (area data fasilitas)
  ...
  const onEachDetectionFeature = useMemo(() => {
    return (feature, layer) => {
      if (!feature.properties) return
      layer.bindPopup(popupDetectionHtml(feature.properties), {
        closeButton: true,
        maxWidth: 260,
        minWidth: 200,
      })
  ...
  const detGeoKey = detectionGeoJSON?.features?.length
    ? `det-${detectionGeoJSON.features.length}-${detectionGeoJSON.properties?.count ?? ''}`
    : ''
      ...
      >
        ...
        {showDetectionLayer &&
          ...
          <GeoJSON
            ...
            data={{
              ...detectionGeoJSON,
              features: detectionGeoJSON.features.filter((f) => f.geometry),
            }}
            style={detectionPolygonStyle}
        ...
        {!loading && (
          <FitBoundsCombined
            ...
            showDetection={Boolean(showDetectionLayer)}
          />
        )}
      </MapContainer>
    </div>
